---
name: export-executive-brief
description: Fourth skill in the SG Semicon US Expansion workflow. Use after qualify-us-prospects generates a 03_qualified_shortlist.json file in the selected SME output folder, or when the user asks to export a qualified shortlist. Read the canonical Step 3 JSON without changing its analysis, create an offline single-file HTML executive dashboard, and render a compact executive summary in the conversation.
---

# Skill: export-executive-brief

Transform canonical qualification JSON artifacts into a zero-friction, board-ready HTML dashboard for Singapore SME leadership and SBF executives. Keep data analysis strictly in Step 3 and presentation in Step 4.

```text
03_qualified_shortlist.json -> Offline HTML Dashboard (Print-to-PDF Ready) + Chat Summary
```

## Inputs

- Required: `output/<safe_sme_name>/03_qualified_shortlist.json`.
- Optional context: `output/<safe_sme_name>/01_capability_profile.md`.

## Outputs

- `output/<safe_sme_name>/04_executive_dashboard.html`.

## Instructions

1. **Accept selected-prompt invocations.** If the message body is blank but selected text contains an instruction for this skill, use the selected text.

2. **Resolve Input Matching.** Use a user-supplied Step 3 JSON path when given. Otherwise locate the most recently modified `output/*/03_qualified_shortlist.json`. If multiple files remain genuinely ambiguous, ask the user to choose. If none exists, notify the user: "No Step 3 qualified shortlist found. Please run `$qualify-us-prospects` first." and stop without creating outputs.

3. **Treat Step 3 JSON as canonical.** Read the complete file and require `sme_name`, `generated_at`, `total_prospects_screened`, `executive_summary`, and `qualified_shortlist`. For schema version `1.4.0`, also require `sbf_engagement_recommendation` with `contact_sbf`, `primary_stage`, `decision_rationale`, `specific_request_to_sbf`, and `sme_preparation_required`; validate its allowed contact decisions and SBF stages. Require `total_prospects_screened` to be an integer of at least 1 and at least the number of shortlisted finalists. Treat `executive_summary.action_now` as the sole immediate-action list. Match every `action_now` and `strategic_routes` prospect to exactly one `qualified_shortlist` prospect using a trimmed, case-insensitive key. Reject empty keys, normalized duplicates, and normalized collisions; never silently merge distinct organizations. If a match is missing or ambiguous, identify it and stop so Step 3 can be corrected. Preserve `total_prospects_screened` and all source names, classifications, roles, clusters, stages, actions, paths, timing signals, evidence descriptions, scores, verification questions, and the SME-level engagement recommendation exactly. Do not browse, rescore, reinterpret, derive immediate actions from classification, or introduce new commercial claims. If required data is missing or malformed, identify the field and stop so Step 3 can be corrected.

4. **Check whether Step 3 may be stale.** Set a `step3_may_be_stale` flag to `false`. If `output/<safe_sme_name>/02_search_log.md` exists, parse `generated_at` from the Step 3 JSON and compare it with every valid ISO 8601 timestamp recorded in the search log's `Timestamp` table cells and `## Scope update: <timestamp>` headings. Compare timezone-aware instants, not timestamp strings. If any recorded Step 2 timestamp is later than `generated_at`, set the flag to `true` and retain the latest such timestamp for the conversation handoff. This check is advisory: continue exporting the canonical Step 3 snapshot without changing it. Do not use file modification times or dates elsewhere in the log as substitutes. If a relevant timestamp cannot be parsed, do not guess; continue the export and report that staleness could not be fully evaluated.

5. **Use the stable dashboard template.** Use `assets/executive-dashboard-template.html` as the complete presentation layer. Do not redesign, restyle, or reconstruct the dashboard, and do not load the files under `references/` during a normal export. Require the template to contain exactly one `__QUALIFIED_SHORTLIST_JSON__` marker.

6. **Embed canonical JSON only.** Serialize the complete Step 3 object without changing field values or array order. Before embedding, replace `<`, `>`, `&`, U+2028, and U+2029 with their JSON Unicode escapes so the payload is safe inside the template's `application/json` script element. Replace the single marker with that escaped JSON and write the result to `output/<safe_sme_name>/04_executive_dashboard.html`. Do not modify any other template text.

7. **Check the rendered artifact contract.** Confirm that the marker is gone, the embedded JSON parses back to an object deeply equal to the Step 3 source, and the output contains no external dependency or network-request code. For schema version `1.4.0`, confirm that the dashboard places the SME-level `Your SBF next step` decision before the KPI cards and renders its contact decision, primary stage, rationale, specific request, and preparation list exactly. For older valid schema versions, show a clearly labelled legacy notice instead of inventing an engagement recommendation. Confirm that the dashboard renders four KPI cards using the preserved `total_prospects_screened`, `qualified_shortlist.length`, `executive_summary.action_now.length`, and unique non-empty shortlist clusters without altering analytical values. Display each prospect's raw score as `N out of 20`; do not display the Step 3 `classification` labels anywhere in the dashboard. In every detailed score breakdown, render each component as plain-language numerator and denominator—for example, `5 out of 5` and `4 out of 4`—and render the overall score as `19 out of 20`; never display bare component numbers or slash-only scores. Order criteria by descending maximum weight, independent of JSON object-key order: 5-point criteria first, then 4-point, then 2-point, then 1-point. In the detailed audit trail, group duplicate normalized evidence URLs for the same finalist into one source card while retaining every role, supported claim, and excerpt from the canonical JSON. Confirm that the template's immediate-action KPI and cards use `executive_summary.action_now`, not classification. Do not add browser, screenshot, or visual-review steps during a normal export.

## Stable template contract

The bundled template owns typography, colors, responsive layout, accessibility, progressive disclosure, score bars, evidence rendering, and print behavior. Its runtime validates Step 3 cross-field matches before rendering. Update the template asset itself when the dashboard design changes; never generate one-off HTML structure in an export run.

## Conversation handoff

After the HTML is written and checked:

1. Verify that the HTML contains no external dependency or network-request references and includes every required section.
2. State the SME-level SBF engagement recommendation before the action table: whether to contact SBF, the primary stage to request, the specific request, and any preparation required. For a valid legacy JSON without this field, state that the dashboard preserves the legacy snapshot and needs a rerun of `$qualify-us-prospects` to generate the SME-level recommendation.
3. Render a compact Markdown table for every `executive_summary.action_now` item in source order with prospect, classification, US cluster, recommended SBF stage, and `next_sbf_action`. If the array is empty, state that Step 3 recorded no immediate-action targets.
4. If `step3_may_be_stale` is `true`, include this note before the final options: *💡 **Note:** Additional discovery searches were logged in Step 2 after your last qualification. If you want these newly discovered prospects included in your dashboard, please run `$qualify-us-prospects` first before exporting.*
5. If staleness could not be fully evaluated because a relevant search-log timestamp was malformed, state that briefly without claiming that Step 3 is current or stale.
6. Provide a clickable absolute local path to `04_executive_dashboard.html`.
7. End with these choices:
   - Double-click `04_executive_dashboard.html` to open it in a browser.
   - Press `Ctrl+P` or `Cmd+P` in the browser to save it as a PDF report.
   - Stop.

## Quality bar

- Keep the HTML fully self-contained and offline-capable.
- Use `executive_summary.action_now` as the single source for the immediate-action KPI, dashboard cards, and conversation table; never rebuild that list from classification.
- Match all scores, claims, and URLs in the canonical JSON exactly; do not add, omit, or alter decision content. Present every component and total score with its denominator in plain language: `5 out of 5`, not merely `5` or `5/5`, and order components by descending maximum weight rather than object-key order. Do not render subjective classification labels; present each prospect's raw `N out of 20` score instead.
- Render each source URL no more than once per finalist in the detailed audit trail; when legacy Step 3 JSON repeats a URL, group its source-backed claims in that one card.
- Keep the interface clean, responsive, printable, highly readable, and executive-ready.
- Keep Step 4 presentation-only. Return analytical corrections to Step 3 rather than silently repairing them here.
