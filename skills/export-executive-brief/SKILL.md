...
7. **Check the rendered artifact contract.** Confirm that the marker is gone, the embedded JSON parses back to an object deeply equal to the Step 3 source, and the output contains no external dependency or network-request code. Confirm that the dashboard renders four KPI cards using the preserved `total_prospects_screened`, `qualified_shortlist.length`, `executive_summary.action_now.length`, and unique non-empty shortlist clusters without altering analytical values. Confirm that the template's immediate-action KPI and cards use `executive_summary.action_now`, not classification. Do not add browser, screenshot, or visual-review steps during a normal export.
