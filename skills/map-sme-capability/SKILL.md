...
* Optional `input/existing_customers.md`: Private user-provided context about the active SME's existing customers and customer patterns.
...
5. **Ingest the source and customer context:** Read the provided website or document. For websites, check the home page plus obvious capability pages such as Services, Products, Solutions, Industries, Certifications, Quality, Equipment, Customers, and About. Ensure `input/existing_customers.md` exists in the current workspace; when missing, copy the bundled `../../input/existing_customers.md` template from the plugin and leave its customer sections empty. Read it when populated. If its `SME name` is blank, set it to the active SME before using any entries. Use populated context only when that name matches the active SME. If it names another SME, stop and ask the user to replace the file or continue without it; never mix customer context between companies. If the website has insufficient customer information and the file has no customer entries, show the Customer Context Check Template and stop. Continue without customer context when the user chooses that option. If the user provides details in chat, offer to record them in the file and proceed when confirmed. Treat all file or chat entries as private user-provided context, not public or independently verified claims. Use named customers and anonymous patterns to understand demonstrated customer fit and routes, but do not use a customer relationship as technical-capability evidence by itself.
...
```text
The public company information does not identify enough existing-customer context to guide prospect discovery.

I created or found: input/existing_customers.md

You may add customer names, anonymous customer patterns, countries, customer types, or routes to market. Named existing customers will be excluded from new-prospect recommendations. Add only information you are authorized to use.
